...
const PHI_FIELDS = ['ssn', 'email', 'phone', 'mrn', 'patientId', 'dateOfBirth', 'address', 'creditCard'];
...
export function auditMiddleware<T>(
  ...
  return (set, get, api) => {
    ...
    const setWithAudit = (partial: T | Partial<T> | ((state: T) => T | Partial<T>), replace?: boolean) => {
