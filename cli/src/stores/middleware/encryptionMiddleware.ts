...
async function decryptObject(obj: unknown): Promise<unknown> {
  ...
  for (const [key, value] of Object.entries(obj)) {
    if (shouldEncryptField(key) && typeof value === 'string' && (obj as Record<string, unknown>)[`${key}_encrypted`]) {
...
export function encryptionMiddleware<T>(
  ...
  return (set, get, api) => {
    ...
    const setWithEncryption = async (partial: T | Partial<T> | ((state: T) => T | Partial<T>), replace?: boolean) => {
