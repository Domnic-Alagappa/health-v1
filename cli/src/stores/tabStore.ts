...
export const useTabStore = create<TabStore>()(
  immer((set, get) => ({
    ...
    getTabByPath: (path: string) => {
      return get().tabs.find((tab) => tab.path === path && tab.id !== DASHBOARD_ID && !tab.accessDenied);
    },
    ...
    openTab: (tab: Omit<Tab, 'id'>, navigate?: (path: string) => void) => {
      ...
      if (!hasAccess && requiredPermission) {
        ...
        set((state) => {
          ...
          state.tabs = dashboard ? [dashboard, accessDeniedTab, ...otherTabs] : [accessDeniedTab, ...otherTabs];
      ...
      set((state) => {
        ...
        if (!tab.allowDuplicate) {
          const existing = state.tabs.find((t) => t.path === tab.path && t.id !== DASHBOARD_ID && !t.accessDenied);
          if (existing) {
        ...
        let label = tab.label;
        if (tab.allowDuplicate) {
          const tabsWithSamePath = state.tabs.filter((t) => t.path === tab.path && t.id !== DASHBOARD_ID);
    ...
    closeTab: (tabId: string, navigate?: (path: string) => void) => {
      set((state) => {
        ...
        if (state.activeTabId === tabId && removedIndex >= 0) {
          ...
          const newActiveTab = prevTabs[removedIndex - 1] || prevTabs[removedIndex + 1] || newTabs[0];
          if (newActiveTab) {
    ...
    reorderTabs: (draggedId: string, targetIndex: number) => {
      set((state) => {
        ...
        for (let i = dashboard ? 1 : 0; i < state.tabs.length; i++) {
          if (state.tabs[i].id !== DASHBOARD_ID) {
            otherTabs.push(state.tabs[i]);
          }
        }
