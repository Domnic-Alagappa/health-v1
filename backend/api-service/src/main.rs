...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let token_manager_for_state = authz_core::oidc::TokenManager::new(
    ...
    let get_permissions_use_case = authz_core::authorization::GetUserPermissionsUseCase::new(
        ...
        Box::new(shared::infrastructure::repositories::PermissionRepositoryImpl::new(database_service.clone())),
    );

    let login_use_case = Arc::new(authz_core::auth::LoginUseCase::new(
        Box::new(shared::infrastructure::repositories::UserRepositoryImpl::new(database_service.clone())),
        Box::new(shared::infrastructure::repositories::RefreshTokenRepositoryImpl::new(database_service.clone())),
        Box::new(shared::infrastructure::repositories::RoleRepositoryImpl::new(database_service.clone())),
        Box::new(shared::infrastructure::repositories::PermissionRepositoryImpl::new(database_service.clone())),
        token_manager_clone,
    ));
    ...
    let relationship_store = Arc::new(shared::infrastructure::zanzibar::RelationshipStore::new(
        Box::new(shared::infrastructure::repositories::RelationshipRepositoryImpl::new(database_service.clone())),
    ));
    ...
    let setup_repository = Arc::new(shared::infrastructure::repositories::SetupRepositoryImpl::new(database_service.clone()));
    ...
    let setup_organization_use_case = Arc::new(admin_service::use_cases::setup::SetupOrganizationUseCase::new(
        Box::new(shared::infrastructure::repositories::SetupRepositoryImpl::new(database_service.clone())),
    ...
    let public_routes = axum::Router::new()
        ...
        .route("/auth/login", axum::routing::post(api_service::presentation::api::handlers::login))
        ...
        .route("/api/services/status", axum::routing::get(api_service::presentation::api::handlers::get_service_status))
        .with_state(app_state_arc.clone());
    
    // Create protected routes with middleware
    let protected_routes = axum::Router::new()
        .route("/auth/logout", axum::routing::post(api_service::presentation::api::handlers::logout))
        .route("/auth/token", axum::routing::post(api_service::presentation::api::handlers::refresh_token))
        .route("/auth/userinfo", axum::routing::get(api_service::presentation::api::handlers::userinfo))
        ...
        .layer(axum::middleware::from_fn_with_state(
            app_state_arc.clone(),
            api_service::presentation::api::middleware::auth_middleware,
        ))
        .layer(axum::middleware::from_fn_with_state(
            app_state_arc.clone(),
            api_service::presentation::api::middleware::acl_middleware,
        ));
    
    let app = axum::Router::new()
        .merge(public_routes)
        .merge(protected_routes)
        .layer(axum::middleware::from_fn(api_service::presentation::api::middleware::request_id_middleware))
